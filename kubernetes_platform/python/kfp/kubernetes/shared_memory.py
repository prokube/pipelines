...
def enable_shared_memory(
    ...
    # Create the EnabledSharedMemory configuration
    enabled_shared_memory = pb.EnabledSharedMemory(
        volume_name=volume_name,
        size=size if size is not None else ""  # Explicitly set to empty if not provided
    )

    # Append the new shared memory configuration to the executor config
    msg.enabled_shared_memory.append(enabled_shared_memory)

    # Convert the updated protobuf message to dictionary and update the task configuration
